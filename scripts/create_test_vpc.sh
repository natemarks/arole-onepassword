#!/usr/bin/env bash
aws cloudformation create-stack --stack-name "test-arole-onepassword" \
...
aws cloudformation wait stack-create-complete --stack-name "test-arole-onepassword"

SUBNET_ID="$(aws cloudformation describe-stacks --stack-name "test-arole-onepassword" --output text | grep 'SubnetPublic0' | awk '{print $9}')"
VPC_ID="$(aws cloudformation describe-stacks --stack-name "test-arole-onepassword" --output text | grep 'VPCID' | awk '{print $6}')"
